...
def convert_video(input_file, output_file):
    try:
        subprocess.run(
            ['ffmpeg', '-i', input_file, '-c:v', 'libx264', '-preset', 'veryslow', '-crf', '18', output_file],
            check=True
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Преобразование видео из кодека VP9 в кодек H.264 с максимальным качеством.')
